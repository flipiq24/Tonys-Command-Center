...
async function main() {
  ...
  try {
    ...
    // 7c. Insert categories
    const categoryUuids = new Map();
    let priorityOrder = 0;
    for (const cat of categories) {
      const id = randomUUID();
      categoryUuids.set(cat, id);
      await client.query(
        `INSERT INTO plan_items (id, level, category, title, status, priority_order, created_at, updated_at)
         VALUES ($1, 'category', $2, $2, 'active', $3, NOW(), NOW())`,
        [id, cat, priorityOrder++],
      );
    }

    // 7d. Insert subcategories
    const subcategoryUuids = new Map(); // key: "cat::sub"
    for (const cat of categories) {
      for (const sub of subcategoriesByCategory.get(cat)) {
        const id = randomUUID();
        subcategoryUuids.set(`${cat}::${sub}`, id);
        await client.query(
          ...
          [id, cat, sub, categoryUuids.get(cat), priorityOrder++],
        );
      }
    }
    ...
    for (const m of masters) {
      ...
      const subUuid = subcategoryUuids.get(`${cat}::${sub}`) || categoryUuids.get(cat) || null;
      ...
      await client.query(
        ...
        [
          id, cat, sub, m["Q2 2026 Task"], description,
          (m["Owner"] || "").trim() || null,
    ...
    for (const s of subs) {
      ...
      await client.query(
        ...
        [
          id, cat, sub, s["Q2 2026 Task"], description,
          (s["Owner"] || "").trim() || null,
