...
// Auth token read via --env-file=.env (Node strips trailing \r from
// Windows-CRLF .env values automatically; manual file parsing does NOT,
// so always rely on process.env here).
const AUTH_TOKEN = process.env.TCC_AUTH_TOKEN || "";

if (!existsSync(FIXTURES_PATH)) {
  console.error(`[replay] ${FIXTURES_PATH} not found. Run seed-synthetic-fixtures.mjs OR capture-chat-fixtures.mjs + label-chat-fixtures.mjs first.`);
  process.exit(1);
}
...
async function main() {
  ...
  // /api/feedback/health is a route added on this branch — used as a litmus
  // test that the running server has the latest agent-runtime code, not just
  // that the port is open.
  try {
    const health = await fetch(`${API_BASE}/api/feedback/health`, { headers: authHeaders() });
    if (health.status === 401) {
      throw new Error("401 Unauthorized — server requires TCC_AUTH_TOKEN. Run with `node --env-file=.env ...` so the token is loaded.");
    }
    if (health.status === 404) {
      throw new Error("404 Not Found — server is running OLD code that lacks /api/feedback/health. Restart the dev server (Ctrl-C + `pnpm dev`) to rebuild with this branch's routes.");
    }
    if (!health.ok) throw new Error(`API server health check failed: ${health.status}`);
  } catch (err) {
    console.error(`[replay] API server not reachable / wrong code at ${API_BASE}.`);
